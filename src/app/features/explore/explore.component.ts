    ...
    @if (catalog.loading()) {
      ...
      <div class="explore container">
        ...
        @if (!activeMood()) {
          ...
          @if (completedMoodCount() > 0) {
            <p class="explore__least-mood">{{ completedMoodCount() }} mood{{ completedMoodCount() > 1 ? 's' : '' }} fully explored!</p>
          }
...
export class ExploreComponent implements OnInit {
  ...
  readonly completedMoodCount = computed(() => {
    const progress = this.watchProgress();
  ...
  readonly unwatchedAvgAge = computed(() => {
    const watchedIds = this.collection.watchedIds();
    if (watchedIds.size === 0) return null;
  ...
  readonly unwatchedAvgYear = computed(() => {
    const watchedIds = this.collection.watchedIds();
    const films = this.catalog.movies().filter((m) => m.isStreamable && !watchedIds.has(m.id));
  ...
  readonly unwatchedCoDirectedCount = computed(() => {
    const watchedIds = this.collection.watchedIds();
    const films = this.catalog.movies().filter((m) => m.isStreamable && !watchedIds.has(m.id));
    if (films.length < 10) return null;
  ...
  readonly unwatchedMedianYear = computed(() => {
    const watchedIds = this.collection.watchedIds();
    const films = this.catalog.movies().filter((m) => m.isStreamable && !watchedIds.has(m.id));
    if (films.length < 10) return null;
  ...
  readonly unwatchedLongestTitle = computed(() => {
    const watchedIds = this.collection.watchedIds();
    const films = this.catalog.movies().filter((m) => m.isStreamable && !watchedIds.has(m.id));
    if (films.length < 10) return null;
    const longest = films.reduce((a, b) => a.title.length >= b.title.length ? a : b);
    return longest.title.length >= 30 ? longest.title : null;
  });

  readonly unwatchedShortestTitle = computed(() => {
    const watchedIds = this.collection.watchedIds();
    const films = this.catalog.movies().filter((m) => m.isStreamable && !watchedIds.has(m.id));
    if (films.length < 10) return null;
  ...
  readonly newestMoodFilm = computed(() => {
    const mood = this.activeMood();
    if (!mood) return null;
    const movies = this.catalog.movies().filter((m) => m.isStreamable && this.matchesMood(m, mood));
    if (movies.length < 3) return null;
    const newest = movies.reduce((a, b) => a.year >= b.year ? a : b);
    return newest.title;
  });

  readonly moodAvgYear = computed(() => {
    const mood = this.activeMood();
    if (!mood) return null;
    const movies = this.catalog.movies().filter((m) => m.isStreamable && this.matchesMood(m, mood));
  ...
  readonly moodSilentEraCount = computed(() => {
    const mood = this.activeMood();
    if (!mood) return null;
    const movies = this.catalog.movies().filter((m) => m.isStreamable && this.matchesMood(m, mood));
    if (movies.length < 5) return null;
    const count = movies.filter((m) => m.year < 1930).length;
    return count >= 3 ? count : null;
  });

  readonly moodNonEnglishCount = computed(() => {
    const mood = this.activeMood();
    if (!mood) return null;
    const movies = this.catalog.movies().filter((m) => m.isStreamable && this.matchesMood(m, mood));
    if (movies.length < 5) return null;
    const count = movies.filter((m) => m.language && m.language !== 'English' && m.language !== 'en').length;
    return count >= 3 ? count : null;
  });

  readonly moodCoDirectedPct = computed(() => {
    const mood = this.activeMood();
    if (!mood) return null;
    const movies = this.catalog.movies().filter((m) => m.isStreamable && this.matchesMood(m, mood));
    if (movies.length < 5) return null;
  ...
  readonly moodImdbLinkedPct = computed(() => {
    const mood = this.activeMood();
    if (!mood) return null;
    const movies = this.catalog.movies().filter((m) => m.isStreamable && this.matchesMood(m, mood));
    if (movies.length < 5) return null;
  ...
  readonly moodPosterCoveragePct = computed(() => {
    const mood = this.activeMood();
    if (!mood) return null;
    const movies = this.catalog.movies().filter((m) => m.isStreamable && this.matchesMood(m, mood));
    if (movies.length < 5) return null;
  ...
  readonly moodAvgDirectorCount = computed(() => {
    const mood = this.activeMood();
    if (!mood) return null;
    const movies = this.catalog.movies().filter((m) => m.isStreamable && this.matchesMood(m, mood));
    if (movies.length < 5) return null;
    const avg = movies.reduce((s, m) => s + m.directors.length, 0) / movies.length;
    return avg >= 1.1 ? avg.toFixed(1) : null;
  });

  readonly avgMoodFilmRating = computed(() => {
    const mood = this.activeMood();
    if (!mood) return null;
    const movies = this.catalog.movies().filter((m) => m.isStreamable && this.matchesMood(m, mood));
  ...
  readonly watchedMoodCount = computed(() => {
    const watchedIds = this.collection.watchedIds();
    if (watchedIds.size < 1) return null;
    const watchedMovies = this.catalog.movies().filter((m) => watchedIds.has(m.id));
    let count = 0;
    for (const mood of this.moods) {
      if (watchedMovies.some((m) => this.matchesMood(m, mood))) count++;
    }
    return count >= 2 ? count : null;
  });

  readonly moodFilmCountRange = computed(() => {
    const movies = this.catalog.movies().filter((m) => m.isStreamable);
    if (movies.length < 10) return null;
    const counts = this.moods.map((mood) => movies.filter((m) => this.matchesMood(m, mood)).length).filter((c) => c > 0);
    if (counts.length < 2) return null;
    const min = Math.min(...counts);
    const max = Math.max(...counts);
    return min !== max ? `${min}–${max} films` : null;
  });

  readonly moodStreamablePct = computed(() => {
    const streamable = this.catalog.movies().filter((m) => m.isStreamable);
    if (streamable.length < 10) return null;
    const moodMatched = streamable.filter((m) => this.moods.some((mood) => this.matchesMood(m, mood))).length;
    const pct = Math.round((moodMatched / streamable.length) * 100);
    return pct > 0 && pct < 100 ? pct : null;
  });

  readonly oldestUnwatchedTitle = computed(() => {
    const watchedIds = this.collection.watchedIds();
    const films = this.catalog.movies().filter((m) => m.isStreamable && !watchedIds.has(m.id));
